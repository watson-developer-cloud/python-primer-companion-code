...
function onProcessClick(urlForAPI){
  ...
  // Flask form doesn't add a id_ prefix to the field ids.
  var ajaxData = "txtdata=" + $('#txtdata').val();
...
function processOK(response) {
  ...
  if (results) {
    ...
    var entity = results['primary_entity'];
    if (entity) {
      $('#id_entity').text(entity); 
    }

    var keyword = results['primary_keyword'];
    if (keyword) {
      $('#id_keyword').text(keyword); 
    }
